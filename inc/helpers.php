...
function acf_responsive_image($image_id,$image_size,$max_width){
	if($image_id != '') {
		...
		echo 'src="'.$image_src.'" srcset="'.$image_srcset.'" sizes="(max-width: '.$max_width.'px) 100vw, '.$max_width.'"';
	}
}
...
/**
 * Finds the common elements between two arrays, only if both arrays are not empty. Returns an empty array otherwise.
 *
